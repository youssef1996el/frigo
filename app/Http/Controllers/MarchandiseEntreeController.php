...
use App\Models\TmpLigneMarchandiseEntree;
...
class MarchandiseEntreeController extends Controller
{
    public function index(Request $request)
    {
        ...
        if($request->ajax())
        {
            ...
            $Data = DB::table('marchandis_entree as m')
                ...
                ->leftJoin(DB::raw('(SELECT id_marchandis_entree, COUNT(*) as total_lignes FROM ligne_marchandis GROUP BY id_marchandis_entree) as lm'),
                        'm.id', '=', 'lm.id_marchandis_entree')
    ...
    public function GetTmpMarchandiseEntreeByUser(Request $request)
    {
        if($request->ajax())
        {
            //get company is active
    ...
    public function StoreMarchandiseEntree(Request $request)
    {
        ...
        $dataTmpMarchandise = TmpLigneMarchandiseEntree::where('idclient', $data['idclient'])
        ...
        // clean temp data
        if ($ligne_marchandise_entree) {
            TmpLigneMarchandiseEntree::where('idclient', $data['idclient'])
                ->where('iduser', $data['iduser'])
                ->where('idcompany', $data['idcompany'])
                ->delete();
    ...
    public function ViewListMarchandiseEntree($id)
    {
        $Data = DB::table('marchandis_entree as m')
        ->join('ligne_marchandis     as l'     , 'l.id_marchandis_entree'   , '=' , 'm.id'      )
        ->leftjoin('list_origins     as list'  , 'list.id'                  , '=' ,'l.idproduct')
        ...
        return view('Marchandise_Entree.list')
        ->with('Data',$Data)
        ->with('id',$id)
        ->with('Clients',$Clients)
        ->with('company',$CompanyIsActive);
    ...
    public function PrintMarchandiseEntree($id)
    {
        ...
        {
            return redirect('MarchandisEntre')->with('ErrorsNumberStartBon','Veuillez choisir le numéro que vous souhaitez strat bon caisse vide');
        }
        ...
        if (!$existingRecord) {
            ...
            {
                $newNumberBon = 1;
                Print_Marchandise_Entre::where('id',$lastRecord->id)->update([
                    'idmarchandise_entree' =>$id
                ]);
            }
            else
            {
                $newNumberBon = $lastRecord ? $lastRecord->number_bon + 1 : 1;
                    Print_Marchandise_Entre::create([
        ...
        try { 
            $Data = DB::table('marchandis_entree as m')
                ...
                ->select(
                    ...
                    DB::raw("SUM(l.quantity) as total_quantity"),
                    DB::raw("MIN(l.id) as idligne"),
                    DB::raw("(SELECT cumul FROM marchandis_entree WHERE id = $id) as cumul")
                )
                ->where('m.id', $id)
                ->groupBy(
                    'c.id',
            ...
            if($Data)
            {
                $marchandise_entree = marchandise_entree::find($id);
                if($marchandise_entree)
                {
                    $marchandise_entree->update([
                        'clotuer'   => true,
                    ]);
                }
            }
            ...
            {
                $html = view('PrintEntreeMarchandise.index', compact('Data','userPrintBon','DatePrintBon','TimePrintBon', 'Extract_number_bon','Client','Info'))->render();
            }
            else
            {
    ...
    public function SaveprintBonMarchandiseEntree(Request $request)
    {
        //get company is active
        $CompanyIsActive = Company::where('status',1)->value('status');
        // get id company is status = 1 
    ...
    public function destroy(Request $request) 
    {
        ...
        try 
        {
            ...
            $idcompany = $entry->idcompany;

            DB::table('ligne_marchandis')->where('id_marchandis_entree',$id)->delete();
            // Delete the entry
            DB::table('marchandis_entree')->where('id', $id)->delete();

            // Recalculate cumul for all remaining rows of the same client and company
            $rows = DB::table('marchandis_entree')
