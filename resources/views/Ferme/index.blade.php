        ...
        <div class="container-fluid">
            ...
            <div class="py-3 d-flex align-items-sm-center flex-sm-row flex-column border bg-light rounded-2 py-2">
                <div class="flex-grow-1">
                    <h4 class="fs-18 fw-semibold m-0">Ferme </h4>
                </div>
                
                <div class="text-end">
                    <ol class="breadcrumb m-0 py-0">
                        <li class="breadcrumb-item"><a href="javascript: void(0);">Apps</a></li>
                        <li class="breadcrumb-item active">Ferme</li>
                    </ol>
                </div>
            </div>
            <div class=" mb-3">
                <a href="{{url('/home')}}" class="btn btn-warning float-end mt-3"      style="margin-right: 5px" >Retour</a>
            </div> 
                        ...
                        <div class="datatable-wrapper datatable-loading no-footer sortable fixed-height d-flex justify-content-center w-100">
                            <div class="datatable-container w-100" >
                                <table class="table datatable datatable-table w-100 " >
                                    <thead>
                                        <tr>
                                            <th colspan="3" class="text-uppercase bg-primary-subtle">Ferme</th>
                                        </tr>
                                        <tr>
                                            <th>Dotation</th>
                                            <th>Charge</th>
                                ...
                                <a href="{{route('ferme.export')}}" class="btn btn-primary" >Export excel</a>
                            </div>
                        </div>
                    </div>
                </div>
                            ...
                            <tr>
                                ...
                                <td>
                                    @if (\Carbon\Carbon::parse($op->operation_date)->diffInHours(now()) < 24)
                                        <a href="{{ url('DeleteFerme/' . $op->idferme) }}" class="btn btn-danger">Supprimer</a>
                                    @endif
                                </td>
                            </tr>
                        @endforeach
            ...
            <div class="table-responsive mt-3">
                @php
                    // Step 1: Determine which columns have data > 0
                    $validColumns = [];
                            ...
                            @foreach($grouped as $date => $chargesValues)
                                <tr>
                                    ...
                                    @foreach($validColumns as $id => $libelle)
                                        <td>
                                            @if(isset($chargesValues[$id]) && $chargesValues[$id] > 0)
                                                {{ $chargesValues[$id] }}
                                            @endif
                                        </td>
                                    @endforeach
